...
*Setup instructions for a gitian build of Segreto using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Segreto
...
- [Building Segreto](#building-segreto)
...
Clone the git repositories for segreto and gitian and then checkout the segreto version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the segreto repository
...
To build Segreto (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the segreto repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/segreto/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/segreto/segreto
...
For example:
```bash
URL=https://github.com/konez2k/segreto.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit segreto=${COMMIT} --url segreto=${URL} ../segreto/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit segreto=${COMMIT} --url segreto=${URL} ../segreto/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit segreto=${COMMIT} --url segreto=${URL} ../segreto/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/segreto-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/segreto-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/segreto-build.assert
```
...
[segreto/gitian.sigs](https://github.com/segreto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
