...
Segreto now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<segreto_address\> \<monthly_payment_segreto\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ SGO, _12_ times over the course of a year totaling _24000_ SGO.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<segreto_address\> \<monthly_payment_segreto\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` SGO.
 ...
 * vote-many          - Vote on a Segreto initiative
 * vote-alias         - Vote on a Segreto initiative
 * vote               - Vote on a Segreto initiative/budget
